...
from ..utils.progress import Silent
from .helpers import (
    cosine_beta_schedule,
    extract,
    apply_conditioning,
    apply_history_conditioning,
    Losses,
    CrossEntropy,
)
...
#         device = self.betas.device
#         batch_size = len(cond[0])
#         # print("conditional_returns: ",returns)
#         horizon = horizon or self.horizon
...
#             # print('x_t: ', x_t.size())
#             # print('a_t :',a_t.size())
#             # print('x_t_1', x_t_1.size())
#             x_comb_t = torch.cat([x_t, x_t_1], dim=-1)
#             x_comb_t = x_comb_t.reshape(-1, 2 * self.observation_dim)
#             a_t = a_t.reshape(-1, self.action_dim)
#             # print('x_cob:',x_comb_t.size())
#             # print('a_treshape: ',a_t.size())
#             if self.ar_inv:
...
class GaussianInvDynDiffusion(nn.Module):
    def __init__(self, model, horizon, history_horizon,observation_dim, action_dim, n_timesteps=1000,
        loss_type='l1', clip_denoised=False, predict_epsilon=True, hidden_dim=256,
        action_weight=1.0, loss_discount=1.0, loss_weights=None,returns_condition=False,
        condition_guidance_w=0.1, ar_inv=False, train_only_inv=False):
        super().__init__()
    ...
    def get_loss_weights(self, discount):
        ...
        if self.history_horizon == 0:
            ...
            loss_weights[self.history_horizon, :] = 0
    ...
    def p_mean_variance(self, x, cond, t, returns=None, cross_attn=None, cross_mask=None):
        if self.returns_condition:
            # epsilon could be epsilon or x0 itself
            epsilon_cond = self.model(x, cond, t, returns, cross_attn, cross_mask, use_dropout=False)
            epsilon_uncond = self.model(x, cond, t, returns, cross_attn, cross_mask, force_dropout=True)
            epsilon = epsilon_uncond + self.condition_guidance_w*(epsilon_cond - epsilon_uncond)
        else:
            epsilon = self.model(x, cond, t, cross_attn, cross_mask)
    ...
    @torch.no_grad()
    def p_sample(self, x, cond, t, returns=None, cross_attn=None, cross_mask=None):
        b, *_, device = *x.shape, x.device
        model_mean, _, model_log_variance = self.p_mean_variance(x=x, cond=cond, t=t, returns=returns, cross_attn=cross_attn, cross_mask=cross_mask)
        noise = 0.5*torch.randn_like(x)
        # no noise when t == 0
        nonzero_mask = (1 - (t == 0).float()).reshape(b, *((1,) * (len(x.shape) - 1)))
        return model_mean + nonzero_mask * (0.5 * model_log_variance).exp() * noise
    ...
    @torch.no_grad()
    def p_sample_loop(self, shape, cond, returns=None, cross_attn=None, cross_mask=None, verbose=True, return_diffusion=False):
        device = self.betas.device

        batch_size = shape[0]
        x = 0.5*torch.randn(shape, device=device)
        if self.history_horizon > 0 :
            x = apply_history_conditioning(x, cond, self.history_horizon, 0)
        else :
            x = apply_conditioning(x, cond, 0)
        if return_diffusion: diffusion = [x]
        ...
        for i in reversed(range(0, self.n_timesteps)):
            timesteps = torch.full((batch_size,), i, device=device, dtype=torch.long)
            x = self.p_sample(x, cond, timesteps, returns, cross_attn, cross_mask)
            if self.history_horizon > 0:
                x = apply_history_conditioning(x, cond, self.history_horizon, 0)
            else : 
                x = apply_conditioning(x, cond, 0)
    ...
    @torch.no_grad()
    def conditional_sample(self, cond, returns=None, cross_attn=None, cross_mask=None, horizon=None, *args, **kwargs):
        '''
            conditions : [ (time, state), ... ]
        '''
        device = self.betas.device
        if self.history_horizon > 0:
            batch_size = len(cond["x"])
        else:
            batch_size = len(cond[0])
        horizon = horizon or self.horizon + self.history_horizon
        shape = (batch_size, horizon, self.observation_dim)

        return self.p_sample_loop(shape, cond, returns, cross_attn, cross_mask, *args, **kwargs)
    #------------------------------------------ training ------------------------------------------#
    ...
    def p_losses(self, x_start, cond, t, returns=None, cross_attn=None, cross_mask=None, diffuse_masks=None):
        noise = torch.randn_like(x_start)

        x_noisy = self.q_sample(x_start=x_start, t=t, noise=noise)
        if self.history_horizon > 0:
            x_noisy = apply_history_conditioning(x_noisy,cond,self.history_horizon,0)
        else :
            x_noisy = apply_conditioning(x_noisy, cond, 0)

        x_recon = self.model(x_noisy, cond, t, returns, cross_attn, cross_mask)

        if not self.predict_epsilon:
            if self.history_horizon > 0:
                x_recon = apply_history_conditioning(x_recon,cond,self.history_horizon,0)
            else :
                x_recon = apply_conditioning(x_recon, cond, 0)
    ...
    def loss(self, x, cond, returns=None,cross_attn=None,cross_mask=None,loss_masks=None,diffuse_masks=None):  
        # if self.train_only_inv:
        #     # Calculating inv loss
        #     x_t = x[:, :-1, self.action_dim:]
        #     a_t = x[:, :-1, :self.action_dim]
        ...
        #     else:
        #         pred_a_t = self.inv_model(x_comb_t)
        #         loss = CrossEntropy(pred_a_t, a_t)
        #         info = {'a0_loss': loss}
        # else:
        batch_size = len(x)
        t = torch.randint(0, self.n_timesteps, (batch_size,), device=x.device).long()
        diffuse_loss, info = self.p_losses(x[:, :, self.action_dim:], cond, t, returns,cross_attn,cross_mask,diffuse_masks)  
        # Calculating inv loss
        x_t = x[:, :-1, self.action_dim:]
        a_t = x[:, :-1, :self.action_dim]
        x_t_1 = x[:, 1:, self.action_dim:]
        ...
        if self.ar_inv:
            ...
            pred_a_t = self.inv_model(x_comb_t)
            inv_loss = CrossEntropy(pred_a_t, a_t, loss_masks)
