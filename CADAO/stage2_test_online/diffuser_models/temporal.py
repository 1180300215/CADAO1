...
class TemporalUnet(nn.Module):
    ...
    def __init__(
        ...
        dims = [transition_dim, *map(lambda m: dim * m, dim_mults)]      # map这个函数的意义是将function应用于iterable的每一个元素，结果以列表的形式返回 (128,512,1024)
        in_out = list(zip(dims[:-1], dims[1:]))    # [:-1]  除了最后一个全取   [1:] 取第二个到最后一个  [(transition, 128), (128, 512), (512, 1024)]
        print(f'[ models/temporal ] Channel dimensions: {in_out}')
        ...
        if self.returns_condition:
            ...
            self.mask_dist = Bernoulli(probs=1-self.condition_dropout)   # 伯努利分布
            embed_dim = 2*dim   # 256
        else:
            embed_dim = dim

        self.downs = nn.ModuleList([])   # 动态的添加或删除子模块，可以包含任意数量的子模块，注册到网络中，参加前向传播反向传播参数优化
        self.ups = nn.ModuleList([])
        num_resolutions = len(in_out)  # 3 

        for ind, (dim_in, dim_out) in enumerate(in_out):
            is_last = ind >= (num_resolutions - 1)  # 是不是最后一个

            self.downs.append(nn.ModuleList([
                ResidualTemporalBlock(dim_in, dim_out, embed_dim=embed_dim, horizon=horizon, kernel_size=kernel_size, mish=mish),
                SpatialTransformer(dim_out, dim_out//num_heads_channels, num_heads_channels, depth=transformer_depth, attn_dim=attn_dim),
                ResidualTemporalBlock(dim_out, dim_out, embed_dim=embed_dim, horizon=horizon, kernel_size=kernel_size, mish=mish),
                SpatialTransformer(dim_out, dim_out//num_heads_channels, num_heads_channels, depth=transformer_depth, attn_dim=attn_dim),
                Downsample1d(dim_out) if not is_last else nn.Identity()
            ]))
        ...
        mid_dim = dims[-1]
        self.mid_block1 = ResidualTemporalBlock(mid_dim, mid_dim, embed_dim=embed_dim, horizon=horizon, kernel_size=kernel_size, mish=mish)
        self.mid_block2 = SpatialTransformer(dim_out, dim_out//num_heads_channels, num_heads_channels, depth=transformer_depth, attn_dim=attn_dim)
        self.mid_block3 = ResidualTemporalBlock(mid_dim, mid_dim, embed_dim=embed_dim, horizon=horizon, kernel_size=kernel_size, mish=mish)
        ...
        for ind, (dim_in, dim_out) in enumerate(reversed(in_out[1:])):
            ...
            self.ups.append(nn.ModuleList([
                ResidualTemporalBlock(dim_out * 2, dim_in, embed_dim=embed_dim, horizon=horizon, kernel_size=kernel_size, mish=mish),
                SpatialTransformer(dim_in, dim_in//num_heads_channels, num_heads_channels, depth=transformer_depth, attn_dim=attn_dim),
                ResidualTemporalBlock(dim_in, dim_in, embed_dim=embed_dim, horizon=horizon, kernel_size=kernel_size, mish=mish),
                SpatialTransformer(dim_in, dim_in//num_heads_channels, num_heads_channels, depth=transformer_depth, attn_dim=attn_dim),
                Upsample1d(dim_in) if not is_last else nn.Identity()
            ]))
    ...
    def forward(self, x, cond, time, returns=None, cross_attn=None, cross_mask=None, use_dropout=True, force_dropout=False):
        '''
            x : [ batch x horizon x transition ]
            returns : [batch x horizon]
        '''
        ...
        for resnet, trans1, resnet2, trans2,downsample in self.downs:
            x = resnet(x, t)
            x = trans1(x,cross_attn,cross_mask)
            x = resnet2(x, t)
            x = trans2(x,cross_attn,cross_mask)
            h.append(x)
            x = downsample(x)

        x = self.mid_block1(x, t)
        x = self.mid_block2(x,cross_attn,cross_mask)
        x = self.mid_block3(x, t)

        # import pdb; pdb.set_trace()

        for resnet, trans1, resnet2, trans2, upsample in self.ups:
            x = torch.cat((x, h.pop()), dim=1)
            x = resnet(x, t)
            x = trans1(x,cross_attn,cross_mask)
            x = resnet2(x, t)
        ...
        x = einops.rearrange(x, 'b t h -> b h t')    # 进行一种转换
    ...
    def get_pred(self, x, cond, time, cross_attn, cross_mask, returns=None, use_dropout=True, force_dropout=False):
        '''
            x : [ batch x horizon x transition ]
            returns : [batch x horizon]
        '''
